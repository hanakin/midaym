...
(function($) {
	    	...
	    	oOverlay.css({
   			...
   			$this.click(function(){
				...
				//reset zoom box prop and add image zoom with a margin top of 15px = imgclose height / 2
   				...
   				if(strTitle){
   					var oTitle = $('<div><dfn class="fancyTitle">'+strTitle+'</dfn></div>').css({
						marginTop:20,
						marginRight:15,
					});
   					oTitle.appendTo(oImgZoomBox);   					
   				}
   				var oImgZoom=$('<img />').attr('src',imgTargetSrc).css({zIndex:999,marginTop:15,marginRight:15}).click(function(){closeZoomBox(o);}).prependTo(oImgZoomBox);
				...
				//be shure that the image to display is loaded open the zoom box, if not display a loading Image.
   				...
   				var fctCalculateImageSize = function () {
   					...
   					var divCalculate = $('<div></div>').css({position:'absolute','top':0,'left':0,opacity:0,'border':'0px solid red',zIndex:199});
	...
	function __displayLoading(){
		if(!oLoading){
			oLoading = $('<div></div>').css({width:50,height:50,zIndex:199,position:'absolute','background':'transparent',
